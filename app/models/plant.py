...
class Plant(db.Model):
    ...
    def to_list_dict(self):
        return {
            'id': self.id,
            'name': self.name,
            'main_image_url': f'/static/{self.main_image_url}' if self.main_image_url else None,
            'likes_count': len(self.users)
        }

    def to_detail_dict(self):
        all_images = [{'id': 0, 'image_url': f'/static/{self.main_image_url}'}] + \
...
class PlantImage(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'image_url': f'/static/{self.image_url}' if self.image_url else None
        }
